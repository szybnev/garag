...
The report records the actual generator and judge model names used for
the run. When they are the same checkpoint, it includes the LLM-as-judge
self-bias caveat; when they differ, it records a general judge caveat
instead.
"""
...
def _render_report(
    ...
    *,
    ...
    if model == judge_model:
        lines.append(
            f"> **Self-bias caveat.** This report uses `{model}` as both generator "
            "and judge, which is known to inflate faithfulness by 5-15% vs a "
            "cross-model judge. Treat the absolute numbers as upper bounds; rely "
            "on per-category deltas and the manual 10-sample review for qualitative "
            "signal. A cross-model rerun (e.g. GPT-4o or Claude as judge) is "
            "deferred."
        )
    else:
        lines.append(
            f"> **Judge caveat.** This report uses `{judge_model}` as an LLM-as-judge "
            f"for answers generated by `{model}`. Treat absolute scores as model-judge "
            "estimates and rely on per-category deltas plus the manual 10-sample "
            "review for qualitative signal."
        )
    lines.append("")
...
def main() -> int:  # noqa: PLR0915
    ...
    if overall["judge_format_failures"]:
        print(
            "[error] judge phase had "
            f"{int(overall['judge_format_failures'])} failures; "
            "not writing generation report"
        )
        return 1
